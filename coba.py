...
if uploaded_file_hour is not None and uploaded_file_day is not None:
    ...
    holiday_workday_cnt = day_df.groupby(["holiday", "workingday"])["cnt"].mean().reset_index()

    plt.figure(figsize=(8, 6))
    sns.barplot(data=holiday_workday_cnt, x="holiday", y="cnt", hue="workingday", palette="mako")
    plt.title("Pengaruh Hari Libur Terhadap Jumlah Penyewaan Sepeda")
    plt.xlabel("Hari Libur")
    plt.ylabel("Rata-rata Jumlah Penyewaan Sepeda")
    plt.legend(title="0 : Kerja, 1: Libur")
    st.pyplot(plt)
