...
# Bagian untuk pertanyaan bisnis
st.header('Pertanyaan Bisnis')
st.write("""
1. Apa perbedaan antara pengguna kasual dan pengguna terdaftar dalam hal jumlah penyewaan per hari?
2. Bagaimana hari libur mempengaruhi jumlah penyewaan sepeda dibandingkan dengan hari kerja?
""")

# Fungsi untuk auto-load dataset dari file sistem
def load_data():
    ...
    if os.path.exists('hour.csv') and os.path.exists('day.csv'):
        ...
        st.error('File hour.csv dan day.csv tidak ditemukan di direktori.')
        return None, None
...
if hour_df is not None and day_df is not None:
    ...
    - Terdapat perbedaan data kolomnya, misalnya pada 'temp', 'attemp', windspeed, dll.
    """)
    ...
    #visualisasi tabel hour_df
    st.write('### Tabel hour_df')
    
    # 1. User Distribution by Hour
    st.subheader("User Distribution by Hour")
    ...
    st.subheader("User Distribution by Weekday")
    ...
    st.subheader("Temperature vs Total Users")
